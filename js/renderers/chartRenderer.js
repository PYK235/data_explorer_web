...
export function renderScatter(
  ...
  const incomes=
    rows.map(
      row=>Number(row.income)
    );

  const spendings=
    rows.map(
      row=>Number(row.spending)
    );
...
function normalizeDendrogramColor(color){
  ...
  if(dendrogramColorMap[color]){
    return dendrogramColorMap[color];
  }

  if(/^C\d$/.test(color)){
    return dendrogramColorMap[color] || "#60a5fa";
  }
...
export function renderDendrogram(
  ...
  if(
    !dendrogram ||
    !Array.isArray(dendrogram.icoord) ||
    dendrogram.icoord.length===0
  ){
    ...
    message.textContent=
      "Chọn Hierarchical để xem dendrogram";
  ...
  const allX=
    dendrogram.icoord.flat();

  const allY=
    dendrogram.dcoord.flat();
  ...
  dendrogram.icoord.forEach(
    (xCoords,idx)=>{

      const yCoords=
        dendrogram.dcoord[idx] || [];

      const path=xCoords
        .map(
          (x,index)=>
            `${xScale(x)} ${yScale(yCoords[index] ?? 0)}`
        )
        .join(" ");
      ...
      svg.append(
        svgNode("polyline",{
          points:path,
          fill:"none",
          stroke:normalizeDendrogramColor(
            dendrogram.color_list?.[idx]
          ),
          "stroke-width":"2"
        })
      );
